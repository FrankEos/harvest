...
public class IpadContentUrlProcessor extends BaseProcessor {
	...
	private static final Logger logger = Logger.getLogger(IpadContentUrlProcessor.class);
	...
	protected boolean content2database(UrlContent urlContent, String content) {
		...
		if (mDBManager.isAppExist(mAppMap, HarvestDBManager.PLATFORM_IPAD)) {
			logger.info(mAppMap.get("title") + " alreay exist , skip it !!!");
			return false;
		}
		// 应用分类
		mAppMap.put("classid", "23");

		mDBManager.insertApp(mAppMap, HarvestDBManager.PLATFORM_IPAD);
		int id = mDBManager.getInsertId(mAppMap, HarvestDBManager.PLATFORM_IPAD);
		...
		if (doDownloadIcon(mAppMap, mAppMap.get("titlepic"))) {
			if (doDownloadImages(mAppMap, mAppMap.get("morepic"))) {
				...
				if (url.startsWith("https://itunes.apple.com") || url.startsWith("http://itunes.apple.com/")) {
					...
					mDBManager.updateDownPath(mAppMap, HarvestDBManager.PLATFORM_IPAD);
					return true;
				}
				// 下载到本地
		...
		mAppMap.put("id", String.valueOf(id));
		if (mDBManager.isIdExist(mAppMap, HarvestDBManager.PLATFORM_IPAD)) {
			mDBManager.deleteById(mAppMap, HarvestDBManager.PLATFORM_IPAD);
		}
		return false;
	}
	...
	private boolean doDownloadIcon(HashMap<String, String> map, String rawUrl) {
		if (!mDBManager.isIdExist(map, HarvestDBManager.PLATFORM_IPAD)) {
			return false;
		}
		...
		if (download(rawUrl, absolutePath)) {
			...
			if (download(rawUrl, absolutePath)) {
				...
				mDBManager.deleteById(map, HarvestDBManager.PLATFORM_IPAD);
				return false;
			}
		}

		mAppMap.put("titlepic", finalPath);
		mDBManager.updateTitlePic(map, HarvestDBManager.PLATFORM_IPAD);
		return true;
	}
	...
	private boolean doDownloadImages(HashMap<String, String> map, String rawUrl) {
		...
		mDBManager.updateMorePic(map, HarvestDBManager.PLATFORM_IPAD);
		return true;
	}
	...
	private boolean doDownloadIpa(HashMap<String, String> map, String rawUrl) {
		...
		if (download(rawUrl, absolutePath)) {
			...
			mDistinctMap.put("file_md5", fileMd5);
			if (mDBManager.isAppExist(mDistinctMap, HarvestDBManager.PLATFORM_IPAD)) {
				file.deleteOnExit();
				finalPath = mDBManager.getAppFilePath(mDistinctMap, HarvestDBManager.PLATFORM_IPAD).replace(TOP_DIR,
						HOST_URL);
			} else {
				mDistinctMap.put("file_abs_path", absolutePath);
				mDistinctMap.put("insert_time", String.valueOf(System.currentTimeMillis() / 1000));
				mDistinctMap.put("file_path", finalPath);
				mDBManager.insertAppMd5(mDistinctMap, HarvestDBManager.PLATFORM_IPAD);
			}
		} else {
		...
		mDBManager.updateIpaDown(map, HarvestDBManager.PLATFORM_IPAD);
	...
	private String compileFilePath(String fileName, String type) {
		...
		return TOP_DIR + File.separatorChar + "iPad" + File.separatorChar + type + File.separatorChar + today[0]
